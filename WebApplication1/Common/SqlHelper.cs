...
namespace paems.Common
{
    ...
    public abstract class SqlHelper
    {
        ...
        /// </summary>
        private static readonly string connectionString = "server=129.211.3.237;database=pae;uid=sa;pwd=Wd123!@#";
        ...
        public static SqlDataReader ExecuteReader(string connectionString, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        {
            ...
            {
                CommonUtils.Nlog().Error(e, $"数据库操作异常-sql:{connectionString}-时间：{DateTime.Now}-参数：{CommonUtils.JSON(commandParameters)}");
                conn.Close();
                throw;
            }
        }
        ...
        public static bool BulkExcute(DataTable dt, string tableName)
        {
            SqlConnection conn = new SqlConnection(connectionString);
            try
            {
            ...
            {
                CommonUtils.Nlog().Error(e, $"数据库操作异常-sql:{connectionString}-时间：{DateTime.Now}");
                conn.Close();
                return false;
            }
        }
